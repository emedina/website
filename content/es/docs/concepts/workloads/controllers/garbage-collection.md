...
El papel del recolector de basura de Kubernetes es el de eliminar determinado objetos
que en algún momento tuvieron un propietario, pero que ahora ya no lo tienen.
...
Algunos objetos de Kubernetes son propietarios de otros objectos. Por ejemplo, un ReplicaSet
...
En ocasiones, Kubernetes pone el valor del campo `ownerReference` automáticamente.
 Por ejemplo, cuando se creas un ReplicaSet, Kubernetes automáticamente pone el valor del campo 
...
También puedes configurar las relacionaes entre los propietarios y sus subordinados
...
{{< note >}}
No se recomienda el uso de referencias de propietario entre espacios de nombre por diseño. Esto quiere decir que: 
1) Los subordinados dentro del ámbito del espacio de nombres sólo pueden definir propietarios en ese mismo espacio de nombres,
...
propietarios dentro del ámbito del espacio de nombres.
{{< /note >}}
...
Fíjate que usando "foregroundDeletion", sólo los subordinados con valor en 
...
en base a los permisos de borrado del objeto propietario, de forma que aquellos subordinados no autorizados
...
Anteriormente a la versión 1.9 de Kubernetes, la regla predeterminada del recolector de basura para la mayoría de controladores era `orphan`.
...
Para eliminar los subordinados automáticamente, pon el parámetro `--cascade` a true.
  Ponlo a false para subordinados huérfanos. Por defecto, el valor de `--cascade`
es true.
