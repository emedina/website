...
Considera la posibilidad de abrir un incidente en el repositorio princial de Kubernetes si tu caso de uso no está soportado por el motivo que sea.
{{< /note >}}
...
* [Crear un Deployment para deplsegar un ReplicaSet](#creating-a-deployment). El ReplicaSet crea los Pods en segundo plano. Comprueba el estado del despliegue para comprobar si es satisfactorio o no.
...
se cambia, por ejemplo si se actualiza las etiquetas o las imáganes de contenedor de la plantilla. 
Otras actualizaciones, como el escalado del Deployment, no conllevan un lanzamiento de despliegue.
{{< /note >}}

Supón que ahora quieres actualizar los Pods nginx para que usen la imagen `nginx:1.9.1`
...
La próxima vez que quieras actualizar estosPods, sólo necesitas actualizar la plantilla Pod del Deployment otra vez.
...
los Pods deseados si no exista ya otro ReplicaSet haciéndolo. Los ReplicaSet existentes que controlan los Pods cuyas etiquetas
...
No se marca ningún ReplicaSet existente como huérfano, y no se crea ningú ReplicaSet nuevo, pero debe tenerse en cuenta que 
...
que todas las actualizadiones se han completado.
...
devuelve un código de salida distinto de 0 si el Deployment ha excecido su tiempo de vencimiento.
...
Puedes configurar el campo `.spec.revisionHistoryLimit` de un Deployment para especificar cuántos viejos ReplicaSets quieres conservar
...
puedes hacerlo creando múltiples Deployments, uno para cada nueva versión, siguiendo el patrón canary descrito en
...
Un Deployment también necesira una [sección `.spec`](https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status).
...
`.spec.strategy` especifica la estrategia usada para remplazar los viejos Pods con los nuevos.
...
Por ejemplo, cuando este valor es 30%, el viejo ReplicaSet puede escalarse al 70% de los 
Pods deseados de forma inmediata tras comenzar el proceso de actualización. Una vez que los Pods están listos, 
el viejo ReplicaSet puede reducirse aún mas, seguido de un escalado del nuevo ReplicaSet, 
...
`.spec.revisionHistoryLimit` es un campo opcional que indica el número de viejos ReplicaSets a retener
para permitir los retrocesos. Estos viejos ReplicaSets consumen recursos en `etcd` y rebosan la salida de `kubectl get rs`. 
La configuración de cada revisión de Deployment se almacena en sus ReplicaSets; 
por lo tanto, una vez que se elimina el viejo ReplicaSet, se pierde la posibilidad de retroceder a dicha revisión del Deployment. 
Por defecto, se retienen hasta 10 viejos ReplicaSets; pero su valor ideal depende de la frecuencia y la estabilidad de los nuevos Deployments.

De forma más específica, si ponemos este campo a cero quiere decir que todos los viejos ReplicaSets con 0 réplicas se limpiarán.
